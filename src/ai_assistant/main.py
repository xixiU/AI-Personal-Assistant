...
class AIAssistant:
    ...
    def _process_event(self, event_data: dict):
        ...
        直接从 event 数据中解析消息并调用 AI，不依赖 adapter 的共享状态，
        避免多线程竞争 adapter.latest_event / latest_message。

        Args:
            event_data: 事件数据，包含 trace_id, adapter, event
        """
        ...
        try:
            ...
            # 直接从 event 中解析消息，不设置 adapter 共享状态
            parsed = self._parse_feishu_event(event, adapter)
            if not parsed:
