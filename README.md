...
- **运维操作助手**：通过 `/运维` 指令远程管理服务器，查看状态、重启应用、分析日志（支持审批流程）
...
**触发方式**（命中任一即进入排查/工具使用模式）：
- **发送日志截图**：带图片的消息自动进入排查模式
- **斜杠指令**：纯文字排查以 `/排查`、`/查代码`、`/code` 开头
- **工具意图关键词**：普通文档检索模式下，（引用）追问中携带"查代码""看源码""排查""调用链""定位问题"等明确的代码/工具意图词，自动切入工具使用模式；文档检索结果仍会作为上下文辅助 AI 判断
- **会话级粘性继承**：同一会话（飞书 chat）一旦进入过工具使用模式，**后续所有追问都自动保持工具使用模式**，无需重复发指令。可持续追问并注入历史（用户提问 / AI 回答 / 用户负反馈）

> **会话粘性说明（修复项）**：早期实现依赖上下文消息窗口里是否残留 `mode==agentic` 的历史回复来判断"追问继承"，但上下文窗口只保留最近若干条消息，多轮追问后最早的排查标记会被挤出窗口，导致第二/第三次追问掉回普通检索模式。现改为**会话级持久标记**（`AnthropicProvider._agentic_sessions`），只要会话进过一次工具模式就一直保持，不再受窗口滑动影响；不同会话之间互相隔离。
...
**触发方式**：发送 `/运维` 或 `/ops` 开头的指令

**核心能力**：
- **远程管理服务器**：通过 SSH 连接执行预定义脚本
- **应用生命周期**：查看状态、启动、停止、重启应用
- **系统监控**：CPU、内存、磁盘、网络使用情况
- **日志查看**：实时查看应用日志，支持 grep 过滤
- **版本查询**：查看应用版本、Git 分支、构建时间（支持 jar/log/file/api 多源）
- **进程分析**：查看端口、线程、内存占用、网络连接

**安全特性**：
- **权限控制**：白名单鉴权（操作者 + 审批人）
- **审批流程**：危险操作（重启/停止）需管理员审批，多人审批（一人同意即可），超时自动取消（默认 3 分钟）
- **命令安全**：预定义脚本路径，参数安全拼接，禁用 shell 解析
...
**技术实现**：
- **SSH 连接池**：使用 paramiko 实现连接复用和自动重连
- **可扩展指令系统**：基于命令模式，用户可自定义指令
- **审批层抽象**：ApprovalProvider 接口，支持飞书/Web/钉钉等多渠道
- **自然语言理解**：AI 可识别机器别名和模糊描述（如"生产1号"→"生产服务器1"）
- **会话粘性**：进入运维模式后追问自动继承，无需重复发 `/运维`

详细配置和使用说明见 `docs/operations-user-guide.md`
...
### 2026-09-10: 工具使用模式会话粘性修复 + 关键词追问触发 + 反馈人昵称展示

**修复 bug（多轮追问工具模式丢失）**：
- **现象**：飞书场景第一次是"查代码"这类工具使用场景，基于第一次回答追问仍能进工具模式，但**基于第二次回答继续追问就掉回普通文档检索模式**。
- **根因**：`_should_use_agentic_mode` 靠遍历上下文消息找 `assistant.metadata.mode=="agentic"` 判断追问继承，而 `ContextManager` 只保留最近 `max_messages`（默认 10）条消息。每轮追问产生 2 条消息，多轮后最早那条带 agentic 标记的回复被挤出窗口，判断失效。
- **修复**：改用**会话级持久标记** `AnthropicProvider._agentic_sessions`（线程安全 set）。会话一旦进入工具模式即记录 session_id，后续所有追问自动继承，不再受上下文窗口滑动影响；不同会话互相隔离。
- **复现步骤（回归验证用）**：飞书发 `/查代码 xxx` → 追问1 → 追问2 → 追问3，全程应保持工具使用模式；换新会话发普通问题应为知识库模式。

**新增功能**：
- **关键词自动切工具模式**：普通检索模式下，（引用）追问携带"查代码/看源码/排查/调用链/定位问题"等工具意图关键词时，自动进入工具使用模式（`_has_tool_intent_keyword`）；文档检索结果仍注入上下文辅助判断。关键词偏保守，避免误伤"报错怎么解决"类纯文档查询。
- **追问历史注入**：持续追问时，历史用户提问 / AI 回答经 `ContextManager` 注入上下文，历史负反馈经 `_build_feedback_prompt` 注入 Agentic 与 RAG 两个分支的 system prompt。
- **反馈人昵称展示**：飞书点赞/点踩时通过通讯录 API（`FeishuBotAdapter.get_user_name`，带内存缓存）获取反馈人昵称，存入反馈记录并在反馈卡片中展示（如"张三 👍 准确"）。需申请 `contact:user.base:readonly` 权限，**未申请自动降级为不展示昵称**，详见下方"申请权限"。

相关文件：`providers/anthropic_provider.py`、`adapters/feishu_bot.py`、`core/feedback_manager.py`、`utils/feishu_message.py`
...
#### 性能优化（提速 60-70%）
**问题**：复杂排查场景（如查实体类对应的表）需要多轮串行工具调用，耗时 200 秒。

**方案 A：Prompt 引导并行**  
在 system prompt 增加"工具使用最佳实践"，引导 Claude 在合适时机主动并行调用工具：
- 不确定用哪个关键词时，一次返回多个 `search_code` 并行执行
- 大文件先 `search_code` 定位行号，再用 `start_line/end_line` 读取上下文
- 明确策略：优先简单关键词，避免复杂正则（git grep 支持有限）

**方案 B：应用层智能批量**  
检测"查实体类对应的表"等高频场景，在 `_execute_tool` 层自动扩展并行任务：
- 检测启发规则：首字母大写、驼峰命名、无正则特殊字符、长度 > 5
- 自动并行 3 个搜索：原始关键词 + `class <实体名>` + `@Table.*<实体名>`
- 若找到唯一 pojo 文件，预读前 30 行（包含 @Table 注解）
- 使用 `ThreadPoolExecutor` 并发执行，结果去重后合并返回

**效果**：
- 查表场景：10 轮 200 秒 → 4-5 轮 60-80 秒（提速 60-70%）
- 应用层并行压缩 5 轮成 1 轮，避免无效正则搜索
...
#### 触发逻辑收紧（避免误触发）
**问题**：用户问"4.3.6 fastjson2 报错怎么解决"因包含"报错"关键词触发 Agentic，实际只是查文档。

**原因**：早期版本根据多种信号自动触发（技术特征、排查关键词、意图关键词、上下文延续），导致普通文档查询误触发。

**改进**：移除所有关键词自动触发，**仅保留两种显式信号**：
1. **斜杠指令**：`/排查`、`/查代码`、`/code`
2. **图片消息**：日志截图

**核心原则**：代码排查是重操作（多轮工具调用、耗时、耗 token），必须由用户显式发起，不做隐式猜测。

**效果对照**：

| 输入 | 之前 | 现在 |
|------|------|------|
| `4.3.6 fastjson2 报错怎么解决` | ❌ Agentic 模式 | ✅ 标准 RAG 模式 |
| `4.3.6 版本部署注意什么` | ❌ Agentic 模式 | ✅ 标准 RAG 模式 |
| `/查代码 fastjson2 序列化错误` | ✅ Agentic 模式 | ✅ Agentic 模式 |
| `[发送日志截图]` | ✅ Agentic 模式 | ✅ Agentic 模式 |

相关提交：`6e77779` `277818b` `15ef91f`

---

#### 其他优化
- **超时模式可配置**：支持 `time`（总时间限制）/ `rounds`（最大轮数）两种超时模式 — `cf2d5a2`
- **多仓库支持**：一个项目可配置多个代码仓库，AI 根据问题自动切换 — `596caa2`
- **体验细节**：飞书即时回复思考表情、@所有人消息过滤、可配置表情类型 — `6593988` `cd4b309` `6ba2300`
...
脚本会自动输出可复制的配置格式，直接粘贴到 `config.yaml` 即可。
...
| `contact:user.base:readonly` | 获取用户基本信息（含姓名） | **反馈人昵称展示**：点赞/点踩时通过 open_id 查询昵称，展示在反馈卡片上 |

> **关于 `contact:user.base:readonly`（可选权限）**：
> 用于在反馈卡片中展示"是谁点了赞/踩"。**未申请此权限也能正常使用**：反馈仍会正常保存，
> 只是卡片上不显示反馈人昵称（自动降级为"👍 准确"而非"张三 👍 准确"）。
> 若需要展示昵称，请在飞书开放平台「权限管理」申请该权限（或以应用身份读取通讯录的
> `contact:contact.base:readonly`），并**重新发布应用版本**使权限生效。
> 权限未生效时，日志会出现 `获取用户昵称失败(可能未申请通讯录权限): code=99991672` 的告警，属正常降级。
